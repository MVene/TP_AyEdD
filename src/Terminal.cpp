...
Terminal::Terminal(pair<int,int> ip) {
    string concatenado = to_string(ip.first) + to_string(ip.second);
    idCompleto = stoi(concatenado);
    ip = ip;
...
void Terminal::recibirPagina(const Pagina& pagina) {
    ...
    cout << "Terminal " << idCompleto << " ha recibido la página " << pagina.id << " completa.\n";
}
...
void Terminal::generarYEnviarPaginas(int cantidadRouters, int cantidadTerminales) {
    ...
    for (int i = 0; i < cantidadPaginas; i++) {
        ...
        cout << "Terminal " << idCompleto << " generó la página " << i << " con " << cantidadPaquetes << " paquetes. \n";
